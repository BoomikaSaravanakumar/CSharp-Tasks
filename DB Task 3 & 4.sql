...
create table CourierServices(ServiceID int constraint pk_sid primary key,ServiceName varchar(100),Cost decimal(8,2),courierid int,
constraint FK_couid foreign key(courierid) references courier(courierid));


--inserting values for CourierService table
insert into CourierServices values(101,'Fast',100.00,001);
insert into CourierServices values(102,'Normal',175.00,002);
insert into CourierServices values(103,'Fast',225.95,003);
insert into CourierServices values(104,'Slow',180.00,004);
insert into CourierServices values(105,'Fast',200.00,005);
insert into CourierServices values(106,'Normal',225.25,006);
insert into CourierServices values(107,'Fast',200.00,007);
insert into CourierServices values(108,'Slow',250.00,008);
insert into CourierServices values(109,'Normal',125.00,009);
insert into CourierServices values(110,'Slow',266.00,010);
insert into CourierServices values(111,'Slow',299.00,011);
...
ContactNumber varchar(20),ERole varchar(50),Salary decimal(10,2),CourierID int ,constraint FK_ID foreign key(courierID) references courier(courierID));

--inserting values for Employee Table
insert into Employee values(1001,'Rasiga','rasi@gmail.com','9087634256','Dispatcher',20000,001);
insert into Employee values(1002,'Kalki','kalki@gmail.com','9065437628','Customer Service Manager ',25000,002);
insert into Employee values(1003,'Yuva','yuva@gmail.com','6534728922','Branch Manager',70000,003);
insert into Employee values(1004,'Nithu','nithu@gmail.com','6756908734','Supervisor',50000,004);
insert into Employee values(1005,'Poorni','poorni@gmail.com','9764538873','Dispatcher',20000,005);
insert into Employee values(1006,'Ragavi','ragavi@gmail.com','9345761299','Delivery Agent',30000,006);
insert into Employee values(1007,'Bhuvi','bhuvi@gmail.com','6954201887','Route Planner',15000,007);
insert into Employee values(1008,'Bhoovi','bhoovi@gmail.com','6954801887','Route Planner',15000,008);
insert into Employee values(1009,'Nimir','nimir@gmail.com','6957801887','Dispatcher',20000,009);
insert into Employee values(1010,'Gokul','gokul@gmail.com','6954202187','Route Planner',15000,010);
insert into Employee values(1011,'Praba','praba@gmail.com','6955362787','supervisor',50000,011);
...
select * from employee;
select * from courier;
select e.employeeid,count(e.courierID) as cou_count from employee e join 
courier c on e.courierID=c.courierid group by e.employeeid;
...
select * from courier;
select * from courierservices;
select s.* from courier c join courierservices s on c.courierid=s.courierid;
...
select e.*,l.* from employee e cross join location_customer l;
...
select * from courier;
select * from courierservices;
select s.* from courierservices s join courier c on s.courierid=c.courierid;

--------------41. Retrieve a list of employees and the number of couriers assigned to each employee:
select * from employee;
select * from courier;
select count(e.courierid)as TotalAssigned, e.employeeid from employee e left join courier c
on e.courierid=c.courierid group by e.employeeid;
...
select * from courier;
select * from employee;
select e.employeeid ,count(e.courierid)as Delivered from employee e left join courier c
on e.courierid=c.CourierID where c.cou_status='delivered' group by e.employeeid;

----------48. Find couriers that were paid an amount greater than the cost of their respective courier services  
select * from courier;
select * from courierservices;
select * from payment;
select c.courierid,p.amount,c.cost from courierservices c left join payment p on
c.courierid=p.courierid where p.amount>c.cost;
...
select distinct locationname from location_customer where ulocationid=(
select ulocationid from payment where amount=(
